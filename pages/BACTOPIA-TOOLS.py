# BACTOPIA-TOOLS.py — Bactopia Local UI (Official tools via --wf)
# ---------------------------------------------------------------------
# • Streamlit interface to orchestrate **Bactopia Tools** (via Nextflow `--wf`)
# • Focused on results already generated by Bactopia (folder with samples)
# • Allows batch execution of official tools such as:
...
# • Asynchronous execution with log tailing (Nextflow)
# • Optional integration with the Bactopia conda environment via BACTOPIA_ENV_PREFIX
...
def bootstrap_bear_env_from_file():
    """
    If the user did not run `source .bear-hub.env` before launching Streamlit,
    try to load this file and populate important variables:
      - BEAR_HUB_ROOT / BEAR_HUB_BASEDIR
      - BACTOPIA_ENV_PREFIX
      - NXF_CONDA_EXE

    Rules:
      - If BACTOPIA_ENV_PREFIX already exists AND NXF_CONDA_EXE points
        to a valid binary, nothing is done.
      - For NXF_CONDA_EXE: if it exists but is invalid, it is overwritten.
      - For the other variables, we only set them if they do not exist yet.
    """
...
def get_nextflow_bin() -> str:
    """
    Priority order for the Nextflow binary:
...
- Distances with `snp-dists` and gene–phenotype association with **Scoary**.
...
- The “AMRFinderPlus extras” field accepts additional raw options.
"""
...
- Main parameters:
  - **`--pf_mincov`**:
    minimum coverage (0–1; e.g. `0.6` = 60%) to consider a hit.
  - **`--pf_threshold`**:
    minimum identity (0–1; e.g. `0.9` = 90%) to consider a hit.
- The “PlasmidFinder extras” field accepts additional options supported by Bactopia Tools.
"""
...
def path_picker(label: str, key: str, mode: str = "dir",
    ...
    """
    Text field + “Browse…” button using `st.dialog` (if available).

    - The pop-up opens only when **Browse…** is clicked.
    - To close the pop-up cleanly, use:
        - “✅ Use this path” or
        - “Cancel”.
    - On older Streamlit versions (no `st.dialog`), falls back to inline mode.
    """
...
def nextflow_available():
    """
    Check if a usable Nextflow is available:
      - st.session_state['nextflow_bin']
      - $NEXTFLOW_BIN
      - BACTOPIA_ENV_PREFIX/bin/nextflow
      - 'nextflow' in PATH
    """
...
def _normalize_linebreaks(chunk: str) -> list[str]:
    """
    Normalize Nextflow log output:
      - remove ANSI colors/escapes
      - break long lines at useful points
    """
    if not chunk:
        return []
...
def start_async_runner_ns(full_cmd: str, ns: str):
    ...
    Start an asynchronous runner:
      - ns = "tools" (namespace prefix in session_state)
      - full_cmd = full bash command to be executed
    """
    log_q = Queue()
    status_q = Queue()
...
if ICON_PATH_BACTOPIA_TOOLS.is_file():
    ...
    st.title("🧰 Bactopia — Official Tools")
...
def discover_samples_from_outdir(outdir: str) -> List[str]:
    ...
    - Strict mode: folders containing 'main/' or 'tools/'.
    - Fallback: if nothing has main/tools, treat any
      non-administrative folder as a sample.
    """
...
def _guess_bt_root_default() -> str:
    """
    Try to guess the base Bactopia folder using a few candidates:

    1) global BEAR-HUB outdir (if it exists in st.session_state['outdir'])
    2) outdir/bactopia_out
    3) PROJECT_ROOT/bactopia_out
    4) APP_ROOT/bactopia_out
    5) ~/BEAR_DATA/bactopia_out (fallback)

    Returns the first folder that has detectable samples.
    """
    candidates: list[pathlib.Path] = []
...
with st.expander("General parameters", expanded=True):
    ...
    help_popover("❓ Help (general parameters)", HELP["gerais"])

# ------------------------- Tool-specific options -------------------------

# ABRicate
if st.session_state.get("bt_run_abricate"):
    st.markdown("**ABRicate — options**")
    st.text_input(
...
if st.session_state.get("bt_run_amrfinderplus"):
    help_header("**AMRFinderPlus — options**", "amrfinderplus")
...
if st.session_state.get("bt_run_rgi"):
    help_header("**RGI (CARD) — options**", "rgi")
...
if st.session_state.get("bt_run_plasmidfinder"):
    help_header("**PlasmidFinder — options**", "plasmidfinder")
    c1, c2 = st.columns(2)
    with c1:
        st.number_input(
            "--pf_mincov (0–1, e.g. 0.6)",
            0.0,
            1.0,
...
if st.session_state.get("bt_run_pangenome"):
    help_header("**Pangenome — main options**", "pangenome")
    st.caption(
    ...
    with cc1:
        st.text_input(
            "--include file (one name per line) [⚠️ overrides automatic include]",
...
if st.session_state.get("bt_run_mashtree"):
    help_header("**Mashtree — options**", "mashtree")
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.number_input(
            "--trunclength",
...
def bt_nextflow_cmd(
    ...
    """
    Build the Nextflow command for a specific Bactopia Tool (`--wf tool`).
    """
...
if stop_tools:
    ...
    status_box_tools.warning("Stop requested…")
        ...
        if not bt_outdir:
            ...
            if not sel:
                ...
                if st.session_state.get("bt_run_pangenome"):
                    ...
                    # Extra filters (⚠️ a second --include may overwrite the global include_file)
