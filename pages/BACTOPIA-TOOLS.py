...
def help_header(title_md: str, help_key: str, ratio=(4,1)):
    ...
    with c1: st.markdown(title_md)
    with c2: help_popover("❓ Ajuda", HELP[help_key])
...
# ============================= Utils =============================
def ensure_state_dir(): APP_STATE_DIR.mkdir(parents=True, exist_ok=True)

def which(cmd: str):
    from shutil import which as _which
    return _which(cmd)

def nextflow_available(): return which("nextflow") is not None

def have_tool(name: str) -> bool: return which(name) is not None

ANSI_ESCAPE = re.compile(r"\x1B\[[0-?]*[ -/]*[@-~]")

def _strip_ansi(s: str) -> str: return ANSI_ESCAPE.sub("", s)
...
# input roots
bt_outdir = st.text_input("Pasta de resultados do Bactopia", value=st.session_state.get("bt_outdir", st.session_state.get("outdir", DEFAULT_OUTDIR)), key="bt_outdir")
samples = discover_samples_from_outdir(bt_outdir) if bt_outdir else []
if samples:
    sel = st.multiselect("Amostras", options=samples,
...
r1 = st.columns(5)
with r1[0]: st.checkbox("amrfinderplus", value=st.session_state.get("bt_run_amrfinderplus", False), key="bt_run_amrfinderplus")
with r1[1]: st.checkbox("rgi",            value=st.session_state.get("bt_run_rgi", False), key="bt_run_rgi")
with r1[2]: st.checkbox("abricate",       value=st.session_state.get("bt_run_abricate", False), key="bt_run_abricate")
with r1[3]: st.checkbox("mobsuite",       value=st.session_state.get("bt_run_mobsuite", False), key="bt_run_mobsuite")
with r1[4]: st.checkbox("mlst",           value=st.session_state.get("bt_run_mlst", False), key="bt_run_mlst")
r2 = st.columns(5)
with r2[0]: st.checkbox("pangenome",      value=st.session_state.get("bt_run_pangenome", False), key="bt_run_pangenome")
with r2[1]: st.checkbox("mashtree",       value=st.session_state.get("bt_run_mashtree", False), key="bt_run_mashtree")
...
if st.session_state.get("bt_run_abricate"):
    ...
    st.text_input("--abricate_db (ex.: ncbi,plasmidfinder)", key="bt_abricate_db", value=st.session_state.get("bt_abricate_db",""))
...
if st.session_state.get("bt_run_amrfinderplus"):
    ...
    with c1:
        ...
        st.checkbox("--mutation_all", value=st.session_state.get("bt_amrfinderplus_mutation_all", False), key="bt_amrfinderplus_mutation_all")
    with c2:
        st.number_input("--ident_min", 0.0, 100.0, value=st.session_state.get("bt_amrfinderplus_ident_min", 90.0), step=0.5, key="bt_amrfinderplus_ident_min")
        st.number_input("--coverage_min", 0.0, 100.0, value=st.session_state.get("bt_amrfinderplus_coverage_min", 50.0), step=0.5, key="bt_amrfinderplus_coverage_min")
    with c3:
        st.text_input("--organism (ex.: Enterobacteriaceae)", value=st.session_state.get("bt_amrfinderplus_organism",""), key="bt_amrfinderplus_organism")
    c4, c5, c6 = st.columns(3)
    with c4:
        st.checkbox("--report_common", value=st.session_state.get("bt_amrfinderplus_report_common", False), key="bt_amrfinderplus_report_common")
    with c5:
        st.checkbox("--report_all_equal_best", value=st.session_state.get("bt_amrfinderplus_report_all_equal_best", False), key="bt_amrfinderplus_report_all_equal_best")
    with c6:
        st.checkbox("--allow_overlap", value=st.session_state.get("bt_amrfinderplus_allow_overlap", False), key="bt_amrfinderplus_allow_overlap")
    st.checkbox("--exclude_quick_need_prediction", value=st.session_state.get("bt_amrfinderplus_exclude_quick_need_prediction", False), key="bt_amrfinderplus_exclude_quick_need_prediction")
    st.text_input("Extras AMRFinderPlus (linha crua/append)", value=st.session_state.get("bt_amrfinderplus_extra",""), key="bt_amrfinderplus_extra")

# --- RGI ---
if st.session_state.get("bt_run_rgi"):
    help_header("**RGI (CARD) — opções**", "rgi")
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.checkbox("--use_diamond", value=st.session_state.get("bt_rgi_use_diamond", True), key="bt_rgi_use_diamond")
    with c2: st.checkbox("--include_loose", value=st.session_state.get("bt_rgi_include_loose", False), key="bt_rgi_include_loose")
    with c3: st.checkbox("--exclude_nudge", value=st.session_state.get("bt_rgi_exclude_nudge", False), key="bt_rgi_exclude_nudge")
    with c4: st.text_input("--frequency (ex.: 'perfect,strict')", value=st.session_state.get("bt_rgi_frequency",""), key="bt_rgi_frequency")
    c5, c6, c7 = st.columns(3)
    with c5: st.text_input("--category", value=st.session_state.get("bt_rgi_category",""), key="bt_rgi_category")
    with c6: st.text_input("--cluster", value=st.session_state.get("bt_rgi_cluster",""), key="bt_rgi_cluster")
    with c7: st.text_input("--display", value=st.session_state.get("bt_rgi_display",""), key="bt_rgi_display")
    st.text_input("Extras RGI (linha crua/append)", value=st.session_state.get("bt_rgi_extra",""), key="bt_rgi_extra")

# --- MLST ---
if st.session_state.get("bt_run_mlst"):
    help_header("**MLST — opções**", "mlst")
    c1, c2, c3, c4, c5 = st.columns(5)
    with c1: st.text_input("--scheme", value=st.session_state.get("bt_mlst_scheme",""), key="bt_mlst_scheme")
    with c2: st.number_input("--minid", 0.0, 100.0, value=st.session_state.get("bt_mlst_minid", 95.0), step=0.5, key="bt_mlst_minid")
    with c3: st.number_input("--mincov", 0.0, 100.0, value=st.session_state.get("bt_mlst_mincov", 50.0), step=0.5, key="bt_mlst_mincov")
    with c4: st.number_input("--minscore", 0.0, 100.0, value=st.session_state.get("bt_mlst_minscore", 0.0), step=0.5, key="bt_mlst_minscore")
    with c5: st.checkbox("--nopath", value=st.session_state.get("bt_mlst_nopath", False), key="bt_mlst_nopath")

# --- Pangenome subset (ATUALIZADO) ---
if st.session_state.get("bt_run_pangenome"):
    help_header("**Pangenome — opções principais**", "pangenome")
    st.caption("Inclui engine (Panaroo/PIRATE/Roary) + IQ-TREE + Prokka + Scoary + SNP-dists. Use 'Extras' gerais para flags adicionais do Nextflow.")

    # Engine
    c1, c2 = st.columns([1,3])
    with c1:
        st.radio("Engine", options=["Panaroo","PIRATE","Roary"],
                 index=["Panaroo","PIRATE","Roary"].index(st.session_state.get("bt_pangenome_engine","Panaroo")),
    ...
    with cc1:
        st.text_input("Arquivo --include (um nome por linha) [⚠️ sobrescreve o include automático]", 
                      value=st.session_state.get("bt_pangenome_include",""), 
                      key="bt_pangenome_include")
    with cc2:
        st.text_input("Arquivo --exclude (um nome por linha)", 
    ...
    c3, c4 = st.columns(2)
    with c3:
        st.text_input("--species (ex.: Escherichia coli)", 
                      value=st.session_state.get("bt_pangenome_species",""), 
                      key="bt_pangenome_species")
    with c4:
        st.text_input("--accessions (arquivo com 1 acesso por linha OU CSV)", 
                      value=st.session_state.get("bt_pangenome_accessions",""), 
                      key="bt_pangenome_accessions")

    # Recombinação
    st.checkbox("--skip_recombination (pular ClonalFrameML)", 
    ...
    with q1: st.text_input("--iqtree_model", value=st.session_state.get("bt_iqtree_model",""), key="bt_iqtree_model")
    with q2: st.number_input("--bb", 0, 5000, value=st.session_state.get("bt_iqtree_bb", 0), step=100, key="bt_iqtree_bb")
    with q3: st.number_input("--alrt", 0, 5000, value=st.session_state.get("bt_iqtree_alrt", 0), step=100, key="bt_iqtree_alrt")
    with q4: st.checkbox("--asr", value=st.session_state.get("bt_iqtree_asr", False), key="bt_iqtree_asr")
    st.text_input("iqtree_opts (append)", value=st.session_state.get("bt_iqtree_opts",""), key="bt_iqtree_opts")
    ...
    if st.session_state.get("bt_pangenome_engine","Panaroo") == "Panaroo":
        ...
        with p1: st.text_input("--panaroo_mode", value=st.session_state.get("bt_panaroo_mode",""), key="bt_panaroo_mode")
        with p2: st.text_input("--panaroo_alignment", value=st.session_state.get("bt_panaroo_alignment",""), key="bt_panaroo_alignment")
        with p3: st.text_input("--panaroo_aligner", value=st.session_state.get("bt_panaroo_aligner",""), key="bt_panaroo_aligner")
        with p4: st.checkbox("--merge_paralogs", value=st.session_state.get("bt_panaroo_merge_paralogs", False), key="bt_panaroo_merge_paralogs")
        p5, p6, p7, p8 = st.columns(4)
        with p5: st.text_input("--panaroo_core_threshold", value=st.session_state.get("bt_panaroo_core_threshold",""), key="bt_panaroo_core_threshold")
        with p6: st.text_input("--panaroo_threshold", value=st.session_state.get("bt_panaroo_threshold",""), key="bt_panaroo_threshold")
        with p7: st.text_input("--panaroo_family_threshold", value=st.session_state.get("bt_panaroo_family_threshold",""), key="bt_panaroo_family_threshold")
        with p8: st.text_input("--len_dif_percent", value=st.session_state.get("bt_panaroo_len_dif_percent",""), key="bt_panaroo_len_dif_percent")
        st.text_input("panaroo_opts (append)", value=st.session_state.get("bt_panaroo_opts",""), key="bt_panaroo_opts")
    ...
    if st.session_state.get("bt_pangenome_engine","Panaroo") == "PIRATE":
        ...
        with r1: st.text_input("--steps (ex.: 50,60,70,80,90,95,98)", value=st.session_state.get("bt_pirate_steps",""), key="bt_pirate_steps")
        with r2: st.text_input("--features (ex.: CDS)", value=st.session_state.get("bt_pirate_features",""), key="bt_pirate_features")
        with r3: st.checkbox("--para_off (desativa identificação de parálogos)", value=st.session_state.get("bt_pirate_para_off", False), key="bt_pirate_para_off")
        with r4: st.checkbox("--z (reter intermediários)", value=st.session_state.get("bt_pirate_z", False), key="bt_pirate_z")
        st.text_input("pan_opt (append p/ engine)", value=st.session_state.get("bt_pirate_opts",""), key="bt_pirate_opts")
    ...
    if st.session_state.get("bt_pangenome_engine","Panaroo") == "Roary":
        ...
        with t1: st.checkbox("--use_prank", value=st.session_state.get("bt_roary_use_prank", False), key="bt_roary_use_prank")
        with t2: st.text_input("--i (identidade %, ex.: 95)", value=st.session_state.get("bt_roary_i",""), key="bt_roary_i")
        with t3: st.text_input("--cd (% core definition, ex.: 99)", value=st.session_state.get("bt_roary_cd",""), key="bt_roary_cd")
        with t4: st.text_input("--g (máx. clusters, ex.: 50000)", value=st.session_state.get("bt_roary_g",""), key="bt_roary_g")
        u1, u2 = st.columns(2)
        with u1: st.checkbox("--s (não separar parálogos)", value=st.session_state.get("bt_roary_s", False), key="bt_roary_s")
        with u2: st.checkbox("--ap (parálogos no core)", value=st.session_state.get("bt_roary_ap", False), key="bt_roary_ap")
        st.text_input("--iv (MCL inflation, ex.: 1.5)", value=st.session_state.get("bt_roary_iv",""), key="bt_roary_iv")

    # Prokka
    help_header("_Prokka (opcional)_", "prokka", ratio=(3,1))
    st.text_input("--proteins (FASTA; opcional)", value=st.session_state.get("bt_prokka_proteins",""), key="bt_prokka_proteins")
    st.text_input("prokka_opts (append)", value=st.session_state.get("bt_prokka_opts",""), key="bt_prokka_opts")

    # Scoary & SNP-dists
    help_header("_Scoary & SNP-dists_", "scoary_snpdists", ratio=(3,1))
    s1, s2, s3, s4 = st.columns(4)
    with s1: st.text_input("--traits (CSV/TSV)", value=st.session_state.get("bt_scoary_traits",""), key="bt_scoary_traits")
    with s2: st.text_input("--p_value_cutoff", value=st.session_state.get("bt_scoary_p_value_cutoff",""), key="bt_scoary_p_value_cutoff")
    with s3: st.text_input("--correction", value=st.session_state.get("bt_scoary_correction",""), key="bt_scoary_correction")
    with s4: st.text_input("--start_col", value=st.session_state.get("bt_scoary_start_col",""), key="bt_scoary_start_col")
...
if st.session_state.get("bt_run_mashtree"):
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.number_input("--trunclength", 0, 1000000, value=st.session_state.get("bt_mashtree_trunclength", 0), step=100, key="bt_mashtree_trunclength")
    with c2: st.text_input("--sortorder (ex.: avg)", value=st.session_state.get("bt_mashtree_sortorder",""), key="bt_mashtree_sortorder")
    with c3: st.number_input("--genomesize", 0, 100000000, value=st.session_state.get("bt_mashtree_genomesize", 0), step=100000, key="bt_mashtree_genomesize")
    with c4: st.number_input("--mindepth", 0, 100, value=st.session_state.get("bt_mashtree_mindepth", 0), step=1, key="bt_mashtree_mindepth")
    c5, c6 = st.columns(2)
    with c5: st.number_input("--kmerlength", 1, 64, value=st.session_state.get("bt_mashtree_kmerlength", 21), step=1, key="bt_mashtree_kmerlength")
    with c6: st.number_input("--sketchsize", 1, 200000, value=st.session_state.get("bt_mashtree_sketchsize", 10000), step=100, key="bt_mashtree_sketchsize")
    st.checkbox("--save_sketches", value=st.session_state.get("bt_mashtree_save_sketches", False), key="bt_mashtree_save_sketches")
...
def write_include_file(outdir: str, samples: List[str]) -> str:
    ...
    with open(fname, "w", encoding="utf-8") as fh:
        for s in samples: fh.write(s+"\n")
    return str(fname)

def bt_nextflow_cmd(tool: str, outdir: str, include_file: str, profile: str,
                    threads: int|None = None, memory_gb: int|None = None,
                    resume: bool = True, extra: List[str] | None = None) -> str:
    base = ["nextflow","run","bactopia/bactopia","-profile",profile,
            "--wf", tool,"--bactopia", outdir,"--include", include_file]
    if threads and threads>0: base += ["--max_cpus", str(threads)]
    if memory_gb and memory_gb>0: base += ["--max_memory", f"{memory_gb}.GB"]
...
with col1:
    start_tools = st.button("▶️ Executar Ferramentas (async)", key="btn_tools_start", disabled=st.session_state.get("tools_running", False))
with col2:
    stop_tools = st.button("⏹️ Interromper", key="btn_tools_stop", disabled=not st.session_state.get("tools_running", False))
    ...
    if not nextflow_available():
        ...
        if not bt_outdir:
            ...
            if not sel:
                ...
                if st.session_state.get("bt_run_amrfinderplus"):
                    extra = []
                    if st.session_state.get("bt_amrfinderplus_plus"): extra.append("--amrfinderplus_plus")
                    if st.session_state.get("bt_amrfinderplus_mutation_all"): extra.append("--amrfinderplus_mutation_all")
                    v = st.session_state.get("bt_amrfinderplus_ident_min")
                    if v not in (None,""): extra += ["--amrfinderplus_ident_min", str(v)]
                    v = st.session_state.get("bt_amrfinderplus_coverage_min")
                    if v not in (None,""): extra += ["--amrfinderplus_coverage_min", str(v)]
                    v = (st.session_state.get("bt_amrfinderplus_organism","")).strip()
                    if v: extra += ["--amrfinderplus_organism", v]
                    if st.session_state.get("bt_amrfinderplus_report_common"): extra.append("--amrfinderplus_report_common")
                    if st.session_state.get("bt_amrfinderplus_report_all_equal_best"): extra.append("--amrfinderplus_report_all_equal_best")
                    if st.session_state.get("bt_amrfinderplus_allow_overlap"): extra.append("--amrfinderplus_allow_overlap")
                    if st.session_state.get("bt_amrfinderplus_exclude_quick_need_prediction"): extra.append("--amrfinderplus_exclude_quick_need_prediction")
                    v = (st.session_state.get("bt_amrfinderplus_extra","")).strip()
                    if v: extra += shlex.split(v)
                    tools_to_run.append(("amrfinderplus", extra))
                if st.session_state.get("bt_run_rgi"):
                    extra = []
                    if st.session_state.get("bt_rgi_use_diamond"): extra.append("--rgi_use_diamond")
                    if st.session_state.get("bt_rgi_include_loose"): extra.append("--rgi_include_loose")
                    if st.session_state.get("bt_rgi_exclude_nudge"): extra.append("--rgi_exclude_nudge")
                    for k, flag in [("bt_rgi_frequency","--rgi_frequency"),("bt_rgi_category","--rgi_category"),("bt_rgi_cluster","--rgi_cluster"),("bt_rgi_display","--rgi_display")]:
                ...
                if st.session_state.get("bt_run_mlst"):
                    ...
                    if v not in (None,""): extra += ["--minid", str(v)]
                    v = st.session_state.get("bt_mlst_mincov")
                    if v not in (None,""): extra += ["--mincov", str(v)]
                    v = st.session_state.get("bt_mlst_minscore")
                    if v not in (None,""): extra += ["--minscore", str(v)]
                    if st.session_state.get("bt_mlst_nopath"): extra.append("--nopath")
                ...
                if st.session_state.get("bt_run_pangenome"):
                    ...
                    if inc: extra += ["--include", inc]
                    if exc: extra += ["--exclude", exc]
                    ...
                    if accessions: extra += ["--accessions", accessions]
                    ...
                    if v: extra += ["--iqtree_model", v]
                    v = st.session_state.get("bt_iqtree_bb",0)
                    if v: extra += ["--bb", str(int(v))]
                    v = st.session_state.get("bt_iqtree_alrt",0)
                    if v: extra += ["--alrt", str(int(v))]
                    if st.session_state.get("bt_iqtree_asr"): extra.append("--asr")
                    v = (st.session_state.get("bt_iqtree_opts","")).strip()
                    if v: extra += ["--iqtree_opts", v]
                    ...
                    if engine == "PIRATE":
                        ...
                        if steps:    extra += ["--steps", steps]
                        if features: extra += ["--features", features]
                        if para_off: extra.append("--para_off")
                        if keep_z:   extra.append("--z")
                        if pan_opt:  extra += ["--pan_opt", pan_opt]

                    # Roary params
                    if engine == "Roary":
                        if st.session_state.get("bt_roary_use_prank"): extra.append("--use_prank")
                        for key, flag in [
                            ("bt_roary_i","--i"),
                            ("bt_roary_cd","--cd"),
                            ("bt_roary_g","--g"),
                            ...
                            vv = (st.session_state.get(key) or "").strip()
                            if vv: extra += [flag, vv]
                        if st.session_state.get("bt_roary_s"):  extra.append("--s")
                        if st.session_state.get("bt_roary_ap"): extra.append("--ap")

                    # Prokka
                    v = (st.session_state.get("bt_prokka_proteins","")).strip()
                    if v: extra += ["--proteins", v]
                    v = (st.session_state.get("bt_prokka_opts","")).strip()
                    if v: extra += ["--prokka_opts", v]

                    # Scoary & SNP-dists
                    v = (st.session_state.get("bt_scoary_traits","")).strip()
                    if v: extra += ["--traits", v]
                    for key, flag in [("bt_scoary_p_value_cutoff","--p_value_cutoff"),("bt_scoary_correction","--correction"),("bt_scoary_start_col","--start_col")]:
                        vv = (st.session_state.get(key) or "").strip()
                        if vv: extra += [flag, vv]
                    if st.session_state.get("bt_scoary_permute"): extra += ["--permute"]
                    if st.session_state.get("bt_snpdists_csv"): extra += ["--csv"]

                    tools_to_run.append(("pangenome", extra))

                if st.session_state.get("bt_run_mashtree"):
                    extra = []
                    for key, flag in [("bt_mashtree_trunclength","--trunclength"),("bt_mashtree_sortorder","--sortorder"),("bt_mashtree_genomesize","--genomesize"),("bt_mashtree_mindepth","--mindepth"),("bt_mashtree_kmerlength","--kmerlength"),("bt_mashtree_sketchsize","--sketchsize")]:
                        vv = st.session_state.get(key)
                        if isinstance(vv, (int,float)) and vv>0: extra += [flag, str(vv)]
                        elif isinstance(vv, str) and vv.strip(): extra += [flag, vv.strip()]
                    if st.session_state.get("bt_mashtree_save_sketches"): extra.append("--save_sketches")
                ...
                if not tools_to_run:
                    ...
                    for tool, extra in tools_to_run:
                        cmdi = bt_nextflow_cmd(tool, bt_outdir, include_file, st.session_state.get("bt_profile","docker"), st.session_state.get("bt_threads") or None, st.session_state.get("bt_memory_gb") or None, resume=st.session_state.get("bt_resume", True), extra=extra)
                        if stdbuf: cmdi = f"{stdbuf} -oL -eL {cmdi}"
