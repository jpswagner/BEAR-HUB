...
- It has been **recovered by decompilation** into `bearhub_rx/_recovered/`:
  ...
  - `_recovered/disasm/` — full `pycdas` bytecode disassembly of the **latest** (3.11)
    build. Authoritative when the decompiled `.py` has a gap: reconstruct logic from here.
...
- This work now lives on the **`reflex` branch** pushed to GitHub — open it from
  `claude.ai/code` or the Claude mobile app to keep editing with full repo context.
